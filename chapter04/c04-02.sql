...
-- 부속질의의 종류
-- 명칭| 위치 | 설명
-- 중첩질의 | where절| where절에 술어와 같이 사용되며 결과를 한정시키기 위해 사용된다
-- 스칼라 부속질의 | select절 | select절에서 사용되며 단일 값을 반환하기 때문에
-- 스칼라 부속질의라고 한다
-- 인라인뷰 | from절 | from절에서 결과를 뷰형캐로 반환하기 때문에 인라인 뷰라고 한다

-- 1. 중첩질의 - whrer 부속 질의
-- 중첩질의 연산자의 종류
-- 술서 | 연산자 | 반환 행 | 반환 열 | 상관
-- 비교 | =,>,<,>=,<= | 단일 | 단일 |기능
-- 집합 | in, notin | 다중 | 다중| 기능
-- 한정 | all,some(any) | 다중 | 단일 | 기능
-- 존재 | exists, not exists | 다중 | 다중 | 필수

-- 비교 연산자
-- 비교연산자는 부속질의가 반드시 단일 행, 단일 영을 반환해야 하며 아닐 경우 질의를
...
-- (최소한 하나라도)이라는 의미를 가진다. any는 some과 동일한 기능을한다
...
-- exists, not exists
-- 존재 연산자인 exists와  not exists 는 데이터의 존재 여부를 확인한다. 주 질의에서
-- 부속질의로 제공된 속성의 값을 가지고 부속질의의 조건을 만족하여 값이 존재하면
-- 참이 된고, 주 질의는 해당 행의 데이터를 출력한 . not exist의 경우 이와 반대로 동작하낟
-- 문법 구조
...
-- dbms는 그중 어떠한 행, 어떠한 열을 출력해야 하는 지 알 수 없어 에러를 출력한다
-- 또 결과가 없는 경우에는 null 값을 출력한다. 일반적으로 select문과 update set 절에 사용된다

--마당서점의 고객별 판매액을 보이시오.(고객이름과 고객별 판매액 출력).
select (select name
        from customer cs
        wher cs.cutid=od.cutid)"name",
        sum(saleprice)"total"
from orders od
group by od.cutid;

-- 스칼라 부속질의 select문과 함께 update 문에서도 사용할 수 있다

-- orders 테이블에 새로운 속성인 도서이름(bookname을 ㅊ우가해보다
alter table orders add bookname varchar2(40);

select * from orders;
-- 새로운 속성에는 null값이 저장되어 있다
-- 스칼라 부속질의를 사용하면 도서 이름을 일과 수정할 수 있다
...
-- 3. 인라뷰 - from 부속질의
-- 뷰는 기존 테이블로부터 일시적으로 만들어진 가상의 테이블을 만한다
